...
contract OptionsVault is AccessControl, IOptions, IStructs {
    ...
    mapping(uint256 => BoolState) public withdrawDelayPeriodLocked;    
    ...
    mapping(uint256 => BoolState) public oracleEnabledLocked;    
    mapping(uint256 => BoolState) public lpWhitelistOnly;     
    mapping(uint256 => BoolState) public buyerWhitelistOnly;     

    OptionsVault1155 public optionsVault1155;
    mapping(IOracle => BoolState) public oracleWhitelisted; 
    mapping(bytes32 => BoolState) public tokenPairWhitelisted; 
    mapping(bytes32 => BoolState) public swapServiceWhitelisted;      
    BoolState public createVaultWhitelisted = BoolState.TrueMutable;
   ...
   function createVault(IOracle _oracle, IERC20 _collateralToken, IERC20 _hedgeToken, IUniswapV2Factory _swapFactory, IUniswapV2Router02 _swapRouter, IFeeCalcs _vaultFeeCalc) public {
        if(boolStateIsTrue(createVaultWhitelisted)){
            require(hasRole(CREATE_VAULT_ROLE, _msgSender()), "OptionsVault: must hold CREATE_VAULT_ROLE");
        }
        require(boolStateIsTrue(oracleWhitelisted[_oracle]),"OptionsVault: oracle must be whitelisted");
        require(boolStateIsTrue(tokenPairWhitelisted[AddressHash(address(_collateralToken),address(_hedgeToken))]),"OptionsVault: token pair must be whitelisted");
        require(boolStateIsTrue(swapServiceWhitelisted[AddressHash(address(_swapFactory),address(_swapRouter))]),"OptionsVault: swap service must be whitelisted");
        ...
        withdrawDelayPeriodLocked[vaultCount] = BoolState.FalseMutable;
        collateralizationRatio[vaultCount] = 10000;
        lpWhitelistOnly[vaultCount] = BoolState.TrueMutable;
        buyerWhitelistOnly[vaultCount] = BoolState.TrueMutable;
        oracleEnabledLocked[vaultCount] = BoolState.FalseMutable;

        
        _setupRole(VAULT_OPERATOR_ROLE(vaultCount), _msgSender());
        _setupRole(VAULT_LPWHITELIST_ROLE(vaultCount), _msgSender());
        _setupRole(VAULT_BUYERWHITELIST_ROLE(vaultCount), _msgSender());
    ...
    function VAULT_BUYERWHITELIST_ROLE(uint _vaultId) public view returns(bytes32){
        return keccak256(abi.encode("VAULT_BUYERWHITELIST_ROLE-", Strings.toString(_vaultId)));
    }
    ...
    function provideAndMint(address account, uint _vaultId, uint256 collateralIn, bool mint) public returns (uint256 mint1155Tokens){
        ...
        if(boolStateIsTrue(lpWhitelistOnly[_vaultId])){            
            require(hasRole(VAULT_LPWHITELIST_ROLE(_vaultId), account), "OptionsVault: must be in LP Whitelist");
        }
    ...
    function isOptionValid(uint _vaultId, address buyer, IOracle _oracle, uint256 optionSize) public view returns (bool) {
        require(!readOnly[_vaultId], "OptionsVault: vault is readonly");
        require(boolStateIsTrue(oracleWhitelisted[_oracle]),"OptionsVault: oracle must be whitelisted");
        require(oracleEnabled[_vaultId][_oracle],"OptionsVault: oracle not enabled for this vault");
        require(boolStateIsTrue(tokenPairWhitelisted[AddressHash(address(collateralToken[_vaultId]),address(hedgeToken[_vaultId]))]),"OptionsVault: token pair must be whitelisted");
        require(boolStateIsTrue(swapServiceWhitelisted[AddressHash(address(swapFactory[_vaultId]),address(swapRouter[_vaultId]))]),"OptionsVault: swap service must be whitelisted");
        if(boolStateIsTrue(buyerWhitelistOnly[_vaultId])){            
            require(hasRole(VAULT_BUYERWHITELIST_ROLE(_vaultId), buyer), "OptionsVault: must be in Buyer Whitelist");
        }
        require(vaultCollateralAvailable(_vaultId)>=optionSize, "OptionsVault: Not enough available collateral");

        return true;
    }

    function boolStateIsTrue(BoolState value) internal view returns (bool){
        return (value==BoolState.TrueMutable || value==BoolState.TrueImmutable);
    }

    function boolStateIsMutable(BoolState value) internal view returns (bool){
        return (value==BoolState.TrueMutable || value==BoolState.FalseMutable);
    }
    ...
    function grantBuyerWhitelistRole(uint vaultId, address value) external {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        emit SetVaultAddress(_msgSender(),SetVariableType.GrantBuyerWhitelistRole, vaultId, address(0), value);
        grantRole(VAULT_BUYERWHITELIST_ROLE(vaultId),value);
    }

    function revokeBuyerWhitelistRole(uint vaultId, address value) external {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        emit SetVaultAddress(_msgSender(),SetVariableType.RevokeBuyerWhitelistRole, vaultId, value, address(0));
        revokeRole(VAULT_BUYERWHITELIST_ROLE(vaultId),value);
    }    
    ...
    function setWithdrawDelayPeriod(uint vaultId, uint256 value) external {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        require(!boolStateIsTrue(withdrawDelayPeriodLocked[vaultId]),"OptionsVault: setting is immutable");
    ...
    function setWithdrawDelayPeriodLockedImmutable(uint vaultId, BoolState value) public {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        require(boolStateIsMutable(withdrawDelayPeriodLocked[vaultId]),"OptionsVault: setting is immutable");
        emit SetVaultBoolState(_msgSender(),SetVariableType.WithdrawDelayPeriodLocked, vaultId, withdrawDelayPeriodLocked[vaultId], value);
        withdrawDelayPeriodLocked[vaultId] = value; 
    }      
    
    function setOracleEnabled(uint vaultId, IOracle _oracle, bool value) external {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        require(boolStateIsTrue(oracleWhitelisted[_oracle]),"OptionsVault: oracle must be whitelisted");
        require(!boolStateIsTrue(oracleEnabledLocked[vaultId]),"OptionsVault: setting is immutable");
    ...
    function setOracleEnabledLockedImmutable(uint vaultId, BoolState value) public {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        require(boolStateIsMutable(oracleEnabledLocked[vaultId]),"OptionsVault: setting is immutable");
        emit SetVaultBoolState(_msgSender(),SetVariableType.OracleEnabledLocked, vaultId, oracleEnabledLocked[vaultId], value);
        oracleEnabledLocked[vaultId] = value; 
    }      

    function setBuyerWhitelistOnly(uint vaultId, bool value) public {
        if (value){
            setBuyerWhitelistOnlyImmutable(vaultId,BoolState.TrueMutable);
        }
        else{
            setBuyerWhitelistOnlyImmutable(vaultId,BoolState.FalseMutable);
        }
    }  

    function setBuyerWhitelistOnlyImmutable(uint vaultId, BoolState value) public {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        require(boolStateIsMutable(buyerWhitelistOnly[vaultId]),"OptionsVault: setting is immutable");
        emit SetVaultBoolState(_msgSender(),SetVariableType.BuyerWhitelistOnly, vaultId, buyerWhitelistOnly[vaultId], value);
        buyerWhitelistOnly[vaultId] = value; 
    }  

    function setLPWhitelistOnly(uint vaultId, bool value) public {
        if (value){
            setLPWhitelistOnlyImmutable(vaultId,BoolState.TrueMutable);
        }
        else{
            setLPWhitelistOnlyImmutable(vaultId,BoolState.FalseMutable);
        }
    }

    function setLPWhitelistOnlyImmutable(uint vaultId, BoolState value) public {
        isVaultOwnerOrOperator(vaultId,_msgSender());
        require(boolStateIsMutable(lpWhitelistOnly[vaultId]),"OptionsVault: setting is immutable");
        emit SetVaultBoolState(_msgSender(),SetVariableType.LPWhitelistOnly, vaultId, lpWhitelistOnly[vaultId], value);
        lpWhitelistOnly[vaultId] = value; 
    }     

    function setOracleWhitelisted(IOracle _oracle, bool value) public {
        if (value){
            setOracleWhitelistedImmutable(_oracle,BoolState.TrueMutable);
        }
        else{
            setOracleWhitelistedImmutable(_oracle,BoolState.FalseMutable);
        }
    }

    function setOracleWhitelistedImmutable(IOracle _oracle, BoolState value) public {
        isDefaultAdmin();        
        require(boolStateIsMutable(oracleWhitelisted[_oracle]),"OptionsVault: setting is immutable");
        emit SetGlobalBoolState(_msgSender(),SetVariableType.OracleWhitelisted, oracleWhitelisted[_oracle], value);
        oracleWhitelisted[_oracle] = value;   
    }  

     function setTokenPairWhitelisted(address _collateralToken, address _hedgeToken, bool value) public { 
        if (value){
            setTokenPairWhitelistedImmutable(_collateralToken,_hedgeToken,BoolState.TrueMutable);
        }
        else{
            setTokenPairWhitelistedImmutable(_collateralToken,_hedgeToken,BoolState.FalseMutable);
        }
     }

     function setTokenPairWhitelistedImmutable(address _collateralToken, address _hedgeToken, BoolState value) public { 
        isDefaultAdmin();        
        require(boolStateIsMutable(tokenPairWhitelisted[AddressHash(_collateralToken,_hedgeToken)]),"OptionsVault: setting is immutable");
        emit SetGlobalAddressPairBoolState(_msgSender(),SetVariableType.TokenPairWhitelisted, _collateralToken, _hedgeToken, tokenPairWhitelisted[AddressHash(_collateralToken,_hedgeToken)], value);
        tokenPairWhitelisted[AddressHash(_collateralToken,_hedgeToken)] = value;         
     }   

     function setSwapSerivceWhitelisted(address _swapFactory, address _swapRouter, bool value) public { 

        if (value){
            setSwapSerivceWhitelistedImmutable(_swapFactory,_swapRouter,BoolState.TrueMutable);
        }
        else{
            setSwapSerivceWhitelistedImmutable(_swapFactory,_swapRouter,BoolState.FalseMutable);
        }
     }

     function setSwapSerivceWhitelistedImmutable(address _swapFactory, address _swapRouter, BoolState value) public { 
        isDefaultAdmin();    
        require(boolStateIsMutable(swapServiceWhitelisted[AddressHash(_swapFactory,_swapRouter)]),"OptionsVault: setting is immutable");
        emit SetGlobalAddressPairBoolState(_msgSender(),SetVariableType.SwapServiceWhitelisted, _swapFactory, _swapRouter, swapServiceWhitelisted[AddressHash(_swapFactory,_swapRouter)], value);
        swapServiceWhitelisted[AddressHash(_swapFactory,_swapRouter)] = value;
     }

    function setCreateVaultWhitelisted(bool value) public {
      if (value){
            setCreateVaultWhitelistedImmutable(BoolState.TrueMutable);
      }
        else{
            setCreateVaultWhitelistedImmutable(BoolState.FalseMutable);
      }
    }


    function setCreateVaultWhitelistedImmutable(BoolState value) public {
        isDefaultAdmin();   
        require(boolStateIsMutable(createVaultWhitelisted),"OptionsVault: setting is immutable");     
        emit SetGlobalBoolState(_msgSender(),SetVariableType.OracleWhitelisted, createVaultWhitelisted, value);
